...
- **Non-boolean objective functions** $\varphi:$ `{0,1, ..., N-1}` $\to [0,1]$, where $N = 2^n$ for a given non-negative integer $n$. In this notebook $\varphi(x) = \pi f(x)$, where $f(x) = \sin^2(x)$.
